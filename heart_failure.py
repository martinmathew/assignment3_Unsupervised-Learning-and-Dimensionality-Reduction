...
def process():
    ...
    feature_selection(X, y, colimns)
    kmean_exp(pca_chose_dimensions_clustering(X,y,10), y, file="elbow_curve_kmeans_after_dim_redheart.png")

    # Kmean Clustering after PCA reduction
    X_red = pca_chose_dimensions_clustering(X, y, 10)
    start = time.time()
    kmean_exp_sillhoute(X_red,y , file="sill_houte_kmeans_after_pca_dim_redheart.png")
    print("K means with PCA feature reduction {}".format(time.time() - start))

    start = time.time()
    silhouette_coefficent(X_red,y,file="k_means_sillhouette_coefficent_after_pca_reduction_{}heart.png")
    ...
    start = time.time()
    #
    X_ica = ica_chose_dimensions_reduction(X, y, 10)
    kmean_exp(X_ica, y, file="elbow_curve_kmeans_after_dim_red_icaheart.png")
    print("K means Elbow with ICA feature reduction {}".format(time.time() - start))
    start = time.time()
    kmean_exp_sillhoute(X_ica, y, file="sill_houte_score_kmeans_after_ica_dim_redheart.png")
    print("K means Sillhoute Score with ICA feature reduction {}".format(time.time() - start))

    start = time.time()
    silhouette_coefficent(X_ica, y, file="sill_houte_coeffiecnt_kmeans_after_ica_dim_red_{}heart.png")
    ...
    X_proj = random_project_reconstruction_with_num_of_cluster(X,y, ncluster=10)
    kmean_exp(X_proj, y, file="elbow_curve_kmeans_after_dim_red_random_projheart.png")
    print("K means Elbow with Randonm Projection feature reduction {}".format(time.time() - start))
    start = time.time()
    kmean_exp_sillhoute(X_proj, y, file="sill_houte_score_kmeans_after_random_proj_dim_redheart.png")
    print("K means Sillhoute Score with Randonm Projection feature reduction {}".format(time.time() - start))

    start = time.time()
    silhouette_coefficent(X_proj, y, file="sill_houte_coeffiecnt_kmeans_after_random_proj_dim_red_{}heart.png")
    ...
    kmean_exp(X_feature, y, file="elbow_curve_kmeans_after_dim_red_feature_projheart.png")
    print("K means Elbow with Feature Selection Projection feature reduction {}".format(time.time() - start))
    start = time.time()
    kmean_exp_sillhoute(X_feature, y, file="sill_houte_score_kmeans_after_feature_selection_dim_redheart.png")
    print("K means Sillhoute Score with Feature Selection feature reduction {}".format(time.time() - start))

    start = time.time()
    silhouette_coefficent(X_feature, y, file="sill_houte_coeffiecnt_kmeans_after_feature_selection_proj_dim_red_{}heart.png")
    ...
    X_red = pca_chose_dimensions_clustering(X, y, 10)
    start = time.time()
    expectation_maximization(X_red,y , file="expectation_maximization_after_pca_dim_redheart.png")
    ...
    X_ica = ica_chose_dimensions_reduction(X, y, 10)
    expectation_maximization(X_ica,y , file="expectation_maximization_after_ica_dim_redheart.png")
    ...
    X_proj = random_project_reconstruction_with_num_of_cluster(X,y, ncluster=10)
    expectation_maximization(X_proj, y, file="expectation_maximization_after_random_proj_dim_redheart.png")
    ...
    X_feature = feature_selection_sort_return(X, y)
    expectation_maximization(X_feature, y, file="elbow_curve_expectation_maximization_after_dim_red_feature_projheart.png")
...
def neural_network_size(new_df, file='neural_network_size_after_clustering'):
    ...
    plt.plot('x_values', 'Cross Validation Score', data=plt_df, marker='o', markerfacecolor='green', markersize=12,
        ...
        # show legend
    plt.legend()
    plt.savefig('{}heart.png'.format(file))
...
def feature_selection(X, y, colimns):
    ...
    pyplot.bar([x for x in colimns], importance)

    pyplot.savefig("feature_selection_XGBheart.png")
...
def random_projection(X, y):
    ...
    plt_df = pd.DataFrame(
        {'x_values': range(1, 12, 3), 'Random Projection': time_random, 'PCA': time_PCA})
    ...
    # show legend
    plt.legend()
    plt.savefig('{}heart.png'.format("runtime_pca_randomheart.png"))
...
def random_projection_recontction_variation(X, y):
    ...
    fig, ax = plt.subplots(figsize=(9, 7))
    plt_df = pd.DataFrame(
        {'x_values': range(1, 10), 'Reconstruction Error': reconstruction})
    ...
    # show legend
    plt.legend()
    plt.savefig('{}heart.png'.format("reconstruction_error_random_proj_varaition"))
...
def random_projection_recontction(X, y):
    ...
    fig, ax = plt.subplots(figsize=(9, 7))
    plt_df = pd.DataFrame(
        {'x_values': range(1, 12, 3), 'Reconstruction Error': reconstruction})
    ...
    # show legend
    plt.legend()
    plt.savefig('{}heart.png'.format("reconstruction_error_random_proj"))
    print("Finished Reconstruction error for randome projection")
...
def ica_chose_dimensions(X, y):
    ...
    plt.grid(False)
    plt.savefig("ica_kurtosisisheart.png")
...
def pca_chose_dimension_plot_eigenvalue_distribution(X, y):
    ...
    plt.savefig('eigen_value_distributionheart.png')
...
def pca_chose_dimensions(X, y):
    ...
    plt.savefig("pca_variance_dimesnionheart.png")
...
def plot_confusion_kmeans(X, y):
    ...
    plt.savefig("kmeans_confusion_matrixheart.png")


def kmean_cluster_visulaize(X, nclusters, file="kmeans_cluster_visualizeheart.png"):
    fig, ax = plt.subplots(figsize=(9, 7))
...
def kmean_exp(X, y, file="elbow_curve_kmeansheart.png"):
    ...
    # plt.savefig("kmeans_clusterheart.png")
...
def kmean_exp_sillhoute(X, y, file="sill_houte_kmeansheart.png"):
    Sill_score = []
    fig, ax = plt.subplots(figsize=(9, 7))
    K = range(2, 10)
    for num_clusters in K:
...
def silhouette_coefficent(X, y, file="k_means_sillhouette_coefficent_{}heart.png"):
...
def expectation_maximization(X, y, file="expectation_maximization_num_of_clustersheart.png"):
    fig, ax = plt.subplots(figsize=(9, 7))
    ...
    plt.errorbar(n_clusters, aics, label='AIC')
